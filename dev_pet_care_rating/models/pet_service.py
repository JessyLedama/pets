...
class dev_pet_service(models.Model):
    ...
    _inherit = ['dev.pet.service','rating.mixin']
    ...
    def _get_rating_value(self):
        for request in self:
            ...
            rating_id = self.env['rating.rating'].search([('res_id','=',self.id),
                                                          ('res_model','=','dev.pet.service')], order='id desc', limit=1)
            if rating_id:
    ...
    def rating_apply(self, rate, token=None, feedback=None, subtype_xmlid=None):
        return super(dev_pet_service, self).rating_apply(rate, token=token, feedback=feedback, subtype_xmlid="dev_pet_care_rating.mt_pet_service_rating")

    def _rating_get_parent_field_name(self):
        return 'partner_id'
    
    def rating_get_partner_id(self):
        res = super(dev_pet_service, self).rating_get_partner_id()
    ...
    def _send_courier_rating_mail(self, force_send=False):
        for request in self:
            template_id = self.env['ir.model.data'].xmlid_to_res_id('dev_pet_care_rating.rating_pet_service_request_email_template', raise_if_not_found=False)
            if template_id:
